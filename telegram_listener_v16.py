...
class TradeManager:
    ...
    def _collect_daily_report_data(self) -> dict:
        ...
        # Map ch_num → canal name — priorité Channels.txt, fallback .env
        ch_name_map = {}
        ...
        closed_positions = {}  # position_id -> {'pnl': float, 'reason': str}
        for deal in all_deals:
            if deal.entry != mt5.DEAL_ENTRY_OUT:
                continue
            if deal.time < start.timestamp():
            ...
            if origin_magic != MAGIC_NUMBER:
                continue
            cp = closed_positions.setdefault(deal.position_id, {'pnl': 0.0, 'reason': 'OTHER'})
            cp['pnl'] += deal.profit
...
async def main():
    ...
    try:
        ...
        if os.path.exists(CHANNELS_TXT_FILE):
            ...
            try:
                _channels_from_file = []  # [(ch_num, name)]
                ...
                _unresolved = []
                for ch_num, name in _channels_from_file:
                    try:
                        # Essayer de trouver le canal par nom dans les dialogs
                        entity = await client.get_entity(name)
                        title_clean = re.sub(r'[\u200b-\u200f\u2028-\u202f\u2060-\u206f\u00a0]', '', name)
                        ...
                        _unresolved.append((ch_num, name))

                # Pour les canaux non résolus par nom, chercher dans les dialogs Telegram
                if _unresolved:
                    log.info(f"{_unresolved.__len__()} canaux non résolus par nom — recherche dans les dialogs...")
                    if TG_FOLDER:
                        try:
                            ...
                            if result:
                                ...
                                # Résoudre chaque peer et matcher avec les noms non résolus
                                _name_to_chnum = {name: ch_num for ch_num, name in _unresolved}
                                for peer in all_peers:
                                    try:
                                        ...
                                        title_clean = unicodedata.normalize('NFKC', title_clean)
                                        # Chercher une correspondance dans les noms non résolus
                                        for unresolved_name, ch_num in _name_to_chnum.items():
                                            ...
                                            if title_clean == _u:
                                                ...
                                                CHANNEL_NUM_MAP[title_clean] = ch_num
                                                CHANNEL_NUM_MAP[str(entity.id)] = ch_num
                                                log.debug(f"Canal_{ch_num} : {title_clean} (résolu via dossier)")
                                                del _name_to_chnum[unresolved_name]
                                                break
                                    except Exception:
                                        continue

                                # Log des canaux toujours non résolus
                                for name, ch_num in _name_to_chnum.items():
