...
class NewsManager:
    ...
    def _check_news(self):
        ...
        for news in self._news:
            ...
            if -after_min <= diff_minutes < 0 and self._blocked:
                remaining = after_min + diff_minutes
                if remaining <= 0:
                    self._blocked = False
                    log.info(msg.log_news_resumed(title))
                    break
            if 0 < diff_minutes <= close_min:
                if not self._blocked:
                    self._blocked = True
                    log.info(msg.log_news_closing_positions(title, diff_minutes))
                    if self.manager:
                        self._close_all()
                    break
                ...
                if not self._blocked:
                    self._blocked = True
                    log.info(msg.log_news_blocking_signals(title, diff_minutes))
                    break
...
async def main():
    ...
    try:
        ...
        log.info(f" SL plus proche : {SL_PLUS_PROCHE}$")
