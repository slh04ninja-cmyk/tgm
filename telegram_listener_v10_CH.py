...
class TradeManager:
    def __init__(self, bridge: MT5Bridge, tracker=None, quick_alerts_ref=None):
        ...
        #   "limit_cas1"  → pending en CAS 1 (annulé par BE déclenché via market_cas1)
        #   "limit_cas2"  → pending en CAS 2-a (annulé par BE déclenché via market_cas2)
        #   "quick_limit" → pending avant remplissage (annulé par BE)
    ...
    def _apply_be_on_open_positions(self, entry: dict, action: str):
        ...
        log.info(msg.log_be_combined(mt5_comment, be_applied, be_price))
        pending_annules = pending_before if (not signal.get("is_single_price", False) and pending_before > 0) else 0

        send_alert_sync(msg.alert_be_activated(action, signal['symbol'], be_applied, be_price, target_gain, canal, pending_annules))
        ...
        for entry in entries_snapshot:
            ...
            if TP_FIXED_ENABLED and not entry.get("_be_activated"):
                if self._check_pnl_trigger(entry):
                    # ★ FIX : cas non-logique — le déclencheur BE se base sur la PIRE
                    # position (PNL_TRIGGER_USD), mais à cet instant précis, le P&L TOTAL
                    # de TOUTES les positions ouvertes (ex: CAS1 avec market pire + limit
                    # bien plus profitable) peut déjà dépasser l'objectif TP-FIXED
                    # (TP_FIXED_GAIN_USD × nb positions). Dans ce cas, fermer DIRECTEMENT
                    # au lieu d'activer le BE et d'attendre le prochain cycle — sinon un
                    # retournement de prix entre les deux vérifications ferait perdre ce
                    # profit déjà disponible.
                    open_tickets_now = [t for t in entry.get("tickets", []) if self._get_pos(t["ticket"])]
                    if open_tickets_now:
                        total_pnl_now = sum(self._get_pos(t["ticket"]).profit for t in open_tickets_now)
                        target_gain_now = TP_FIXED_GAIN_USD * len(open_tickets_now)
                        if total_pnl_now >= target_gain_now:
                            entry["_be_activated"] = True
                            entry["_target_gain"] = target_gain_now
                            self._close_entry_tp_fixed(entry, action, symbol, mt5_comment, canal, open_tickets_now, total_pnl_now)
                            continue
                        ...
                        for t in unprotected:
                            ...
                            if pos:
                                ...
                                if self.bridge.modify_sl(t["ticket"], new_be_rounded, f"[BE LATE @{new_be_rounded}]"):
                                    ...
                                    log.info(msg.log_be_combined(mt5_comment, open_count, new_be_rounded))
                                    send_alert_sync(msg.alert_be_late(action, symbol, new_be_rounded, open_count, TP_FIXED_GAIN_USD * open_count, TP_FIXED_GAIN_USD * (open_count - 1), canal))
            ...
            if entry.get("_be_activated"):
                ...
                if target_gain > 0:
                    ...
                    if total_pnl >= target_gain:
                        self._close_entry_tp_fixed(entry, action, symbol, mt5_comment, canal, active_tickets, total_pnl)
                        continue
...
def execute_signal(signal: dict, bridge: MT5Bridge, manager, tracker):
    ...
    if is_single_price and len(all_tps) >= 1:
        ...
        if scenario == 3:
            log.info(msg.log_refuse(ch_num, "-PU", msg.MOTIF_PRIX_HORS_ZONE))
            log.info(msg.log_annule(mt5_comment_pu, f"prix={current} hors zone S2 | entry={entry_price} TP1={tp1} SL={sl_price}"))
            return
        ...
        if not orders and not tickets:
            log.info(msg.log_refuse(ch_num, "-PU", msg.MOTIF_ECHEC_PLACEMENT))
    ...
    if in_zone:
        ...
        # ★ FIX : limit_price ancré sur le bord de la zone, PAS sur le SL brut du signal.
        # Avant : (sl + zone_edge) / 2 — un SL large du signal poussait le LIMIT loin de la
        # zone, ce qui contaminait ensuite le calcul du SL final (ancré sur ce LIMIT déjà faux).
        if action == "BUY":
            limit_price = round(zone_low, sym_info.digits)
        else:
            limit_price = round(zone_high, sym_info.digits)
        ...
        # ★ SL_TOTAL : perte totale fixe si les 2 jambes sont présentes,
        # sinon on garde le SL brut du signal (une seule jambe placée, cas improbable).
        if t and o:
            sl = compute_sl_total(market_entry_price, limit_price, action)
        if t:
        ...
        if between_zone_tp1:
            ...
            # sinon on garde le SL brut du signal (une seule jambe placée, cas improbable).
            if t and o:
                sl = compute_sl_total(current, other_limit, action)
            if t:
            ...
            # sinon on garde le SL brut du signal (une seule jambe placée, cas improbable).
            if o1 and o2:
                sl = compute_sl_total(price_1, price_2, action)
            if o1:
            ...
            log.info(msg.log_refuse(ch_num, "-C", msg.MOTIF_PRIX_ATTEINT_TP3))
            log.debug(f"CAS 2 — prix hors zone → ANNULÉ | prix={current} | TP1={tp1} TP2={tp2}")
            return

    if not orders and not tickets:
        log.info(msg.log_refuse(ch_num, "-C", msg.MOTIF_ECHEC_PLACEMENT))
    ...
    for t in tickets:
        ...
        if 'market' in role:
            ...
            label = role.upper()
        parts_data.append((label, t['ticket'], t['entry_price']))
    for o in orders:
        role = o['role']
        if 'market' in role:
            label = 'MKT'
            ...
            label = role.upper()
        parts_data.append((label, o['order'], o['price']))
    if len(parts_data) == 1:
        log.info(msg.log_order_placed(mt5_comment, parts_data[0][0], parts_data[0][1], parts_data[0][2], sl))
    elif len(parts_data) >= 2:
        log.info(msg.log_order_placed_dual(mt5_comment, parts_data[0][0], parts_data[0][1], parts_data[0][2],
                                            parts_data[1][0], parts_data[1][1], parts_data[1][2], sl))
...
def merge_quick_alert(qa: dict, key: str, full_signal: dict,
    ...
    if not qa_is_limit:
        ...
        # ★ Placer le merge limit d'abord pour obtenir le SL_TOTAL correct
        merge_ticket, merge_sl = _place_merge_limit(full_signal, bridge, entry, real_sl, tp_final)
        # Appliquer le SL du merge (SL_TOTAL) sur la position MP-MKT existante
        ...
        if resolved_pos:
            ...
            merge_ticket, merge_sl = _place_merge_limit(full_signal, bridge, entry, real_sl, tp_final)
            # Appliquer le SL du merge (SL_TOTAL) sur la position remplie
            ...
            for o in entry["orders"]:
                if o["order"] == qa_ticket:
                    ...
                    break
            merge_ticket, merge_sl = _place_merge_limit(full_signal, bridge, entry, real_sl, tp_final)
            # Appliquer le SL du merge (SL_TOTAL) sur l'ordre pending
