...
=============================================================
 ...
 Version 17.0.0 — Bias Filters (5 Edges Mesurables)
=============================================================
MODIFICATIONS v17.0.0 (depuis v16.0.0) :

■ NOUVEAU : 5 filtres de biais mesurables
- GLD ETF Flow : flux institutionnel quotidien
- Shanghai Premium : demande physique Chine
- COT Report : positionnement institutionnel (hebdo)
- GVZ : Gold Volatility Index (contrarian)
- Fed Funds : attentes de taux
- Chaque filtre activable/désactivable dans 17.env
- Score composite (-10 à +10) → LONG / NEUTRAL / SHORT
- Mode STRICT (rejet) ou SOUPLE (log seulement)

■ HÉRITÉ DE V16 :
- 5 positions par signal (A/B Testing P1-P4b)
- SL plafonné (MAX_SL_USD)
- Rapport quotidien (texte + PDF)
- News filter par tiers
- Fix P&L quotidien recalculé depuis MT5
=============================================================
"""
...
from bias_filters import BiasFilterEngine, BiasResult, init_bias_engine, get_bias_engine
...
async def main():
    ...
    try:
        ...
        # =============================================================
        # ★ V17 : INITIALISATION DES BIAS FILTERS
        # =============================================================
        bias_engine = init_bias_engine(os.environ)
        enabled_count = sum([
            bias_engine.gld_enabled,
            bias_engine.shanghai_enabled,
            bias_engine.cot_enabled,
            bias_engine.gvz_enabled,
            bias_engine.fed_enabled,
        ])
        log.info(f"[BIAS] Moteur initialisé — {enabled_count}/5 filtres activés")

        # Calculer le biais initial
        bias_result = bias_engine.calculate_bias()
        log.info(bias_engine.format_bias_log(bias_result))
        if bias_result.bias != "NEUTRAL":
            send_alert_sync(bias_engine.format_bias_alert(bias_result))

        # Dernier calcul de biais (pour rafraîchissement périodique)
        _last_bias_calc = time.time()
        _BIAS_REFRESH_SEC = 3600  # recalculer toutes les heures
        # Note: _last_bias_calc est mutable via liste pour le closure dans le handler
        _bias_state = {'last_calc': time.time(), 'result': bias_result}
        ...
        async def handler(event):
            ...
            if signal_data.signal_type == "CLOSE":
                ...
                if enabled_count > 0 and bias_result.direction != "BOTH":
                    signal_action = signal_data.direction or ""
                    allowed, bias_reason = bias_engine.check_signal(signal_action, bias_result)
                    if not allowed:
                        log.info(f"CH{_sig_ch_num}-{_sig_mode} | REFUSÉ BIAS {bias_result.bias} (score={bias_result.total_score:+d})")
                        _alert_mgmt(f"🚫 BIAS {bias_result.bias} | {signal_action} {signal_data.pair} | score={bias_result.total_score:+d} | {bias_reason}")
                        return
                    elif "STRONG" in bias_result.bias:
                        log.debug(f"[BIAS] {signal_action} accepté malgré {bias_result.bias} (mode souple)")
